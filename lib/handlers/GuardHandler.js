      ...
      if (request.session) {
        ...
        guard.resume(session.id, function (error, updatedSession) {
          if (error) {
            ...
            var domain = GuardHandler.cookieDomain(request.headers.origin || '');

            // Redirect to index immediatly
            response.writeHead(200, {
              'Set-Cookie': [
                _sessionKey + '=' + updatedSession.id + '; domain=' + domain + '; Expires=' + expiresText + '; Path=/;'
              ]
            });
          ...
          guard.authenticate(
            ...
            function (error, session) {
              ...
              if (!error) {
                ...
                response.writeHead(200, {
                  'Set-Cookie': [
                    'user=' + JSON.stringify(session.user) + '; domain=' + domain + '; Expires=' + expiresText + '; Path=/;',
                    _sessionKey + '=' + session.id + '; domain=' + domain + '; Expires=' + expiresText +'; Path=/;'
                  ]
                });
                response.end(JSON.stringify({ 
                  action: 'authenticate',

      ...
      if (request.session) {
        ...
        guard.resume(session.id, function (error, updatedSession) {
          if (error) {
            ...
            response.writeHead(200, {
              'Set-Cookie': [
                _sessionKey + '=' + updatedSession.id + '; Expires=' + expiresText + '; Path=/; Domain=' + domain + ';',
          ...
          guard.authenticate(
            ...
            function (error, session) {
              ...
              if (!error) {
                ...
                response.writeHead(200, {
                  'Set-Cookie': [
                    'user=' + JSON.stringify(session.user) + '; Expires=' + expiresText + '; Path=/; Domain=' + domain + ';',
                    _sessionKey + '=' + session.id + '; Expires=' + expiresText +'; Path=/; Domain=' + domain + ';',
                    'domain=' + request.headers.origin + ';'
                  ]
                });
                response.end(JSON.stringify({ 

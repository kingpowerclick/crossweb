...
Router.prototype.invoke = function (request, response) {
  ...
  var callback = function (error, output) {
    ...
    if (error || !output) {
      ...
      filter.check(request, response, callback);
    } else {
    ...
    function (callback) {
      ...
      try {
        if (fs.existsSync(modulePath) || 
            fs.existsSync(modulePathJS)) {
          loadedModule = require(modulePath);
        } else if (fs.existsSync(frameworkModulePath) ||
                   fs.existsSync(frameworkModulePathJS)) {
          ...
          async.whilst(
            ...
            function (callback) {
              ...
              if (pattern.test(url)) {
                ...
                if (!fs.existsSync(handlerPath) && !fs.existsSync(handlerJSPath)) {
